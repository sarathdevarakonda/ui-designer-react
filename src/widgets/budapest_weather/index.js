...
const WeatherLoaded = "weather/budapest";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            budapestWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=budapest&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.budapestWeather || !state.budapestWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.budapestWeather.weather.name,
        temperature: Math.round(state.budapestWeather.weather.main.temp - 273),
        description: state.budapestWeather.weather.weather[0].description,
    };
};
