...
const WeatherLoaded = "weather/seoul";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=seoul&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.seoulWeather || !state.seoulWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.seoulWeather.weather.name,
        temperature: Math.round(state.seoulWeather.weather.main.temp - 273),
        description: state.seoulWeather.weather.weather[0].description,
    };
};
