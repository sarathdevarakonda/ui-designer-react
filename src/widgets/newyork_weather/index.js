...
const WeatherLoaded = "weather/newyork";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=newyork&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.newyorkWeather || !state.newyorkWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.newyorkWeather.weather.name,
        temperature: Math.round(state.newyorkWeather.weather.main.temp - 273),
        description: state.newyorkWeather.weather.weather[0].description,
    };
};
