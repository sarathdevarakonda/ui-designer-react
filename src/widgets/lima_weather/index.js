...
const WeatherLoaded = "weather/lima";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=lima&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.limaWeather || !state.limaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.limaWeather.weather.name,
        temperature: Math.round(state.limaWeather.weather.main.temp - 273),
        description: state.limaWeather.weather.weather[0].description,
    };
};
