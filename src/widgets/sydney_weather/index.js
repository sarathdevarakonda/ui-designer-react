...
const WeatherLoaded = "weather/sydney";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=sydney&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.sydneyWeather || !state.sydneyWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.sydneyWeather.weather.name,
        temperature: Math.round(state.sydneyWeather.weather.main.temp - 273),
        description: state.sydneyWeather.weather.weather[0].description,
    };
};
