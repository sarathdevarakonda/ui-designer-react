...
const WeatherLoaded = "weather/paris";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=paris&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.parisWeather || !state.parisWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.parisWeather.weather.name,
        temperature: Math.round(state.parisWeather.weather.main.temp - 273),
        description: state.parisWeather.weather.weather[0].description,
    };
};
