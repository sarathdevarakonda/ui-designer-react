...
const WeatherLoaded = "weather/kiev";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=kiev&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.kievWeather || !state.kievWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.kievWeather.weather.name,
        temperature: Math.round(state.kievWeather.weather.main.temp - 273),
        description: state.kievWeather.weather.weather[0].description,
    };
};
