...
const WeatherLoaded = "weather/cairo";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=cairo&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.cairoWeather || !state.cairoWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.cairoWeather.weather.name,
        temperature: Math.round(state.cairoWeather.weather.main.temp - 273),
        description: state.cairoWeather.weather.weather[0].description,
    };
};
