...
const WeatherLoaded = "weather/santiago";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            santiagoWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=santiago&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.santiagoWeather || !state.santiagoWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.santiagoWeather.weather.name,
        temperature: Math.round(state.santiagoWeather.weather.main.temp - 273),
        description: state.santiagoWeather.weather.weather[0].description,
    };
};
