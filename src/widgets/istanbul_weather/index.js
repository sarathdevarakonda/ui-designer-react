...
const WeatherLoaded = "weather/istanbul";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            istanbulWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=istanbul&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.istanbulWeather || !state.istanbulWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.istanbulWeather.weather.name,
        temperature: Math.round(state.istanbulWeather.weather.main.temp - 273),
        description: state.istanbulWeather.weather.weather[0].description,
    };
};
