...
const WeatherLoaded = "weather/jakarta";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=jakarta&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.jakartaWeather || !state.jakartaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.jakartaWeather.weather.name,
        temperature: Math.round(state.jakartaWeather.weather.main.temp - 273),
        description: state.jakartaWeather.weather.weather[0].description,
    };
};
