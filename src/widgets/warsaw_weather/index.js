...
const WeatherLoaded = "weather/warsaw";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=warsaw&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.warsawWeather || !state.warsawWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.warsawWeather.weather.name,
        temperature: Math.round(state.warsawWeather.weather.main.temp - 273),
        description: state.warsawWeather.weather.weather[0].description,
    };
};
