...
const WeatherLoaded = "weather/vienna";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=vienna&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.viennaWeather || !state.viennaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.viennaWeather.weather.name,
        temperature: Math.round(state.viennaWeather.weather.main.temp - 273),
        description: state.viennaWeather.weather.weather[0].description,
    };
};
