...
const WeatherLoaded = "weather/toronto";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=toronto&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.torontoWeather || !state.torontoWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.torontoWeather.weather.name,
        temperature: Math.round(state.torontoWeather.weather.main.temp - 273),
        description: state.torontoWeather.weather.weather[0].description,
    };
};
