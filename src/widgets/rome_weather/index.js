...
const WeatherLoaded = "weather/rome";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=rome&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.romeWeather || !state.romeWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.romeWeather.weather.name,
        temperature: Math.round(state.romeWeather.weather.main.temp - 273),
        description: state.romeWeather.weather.weather[0].description,
    };
};
