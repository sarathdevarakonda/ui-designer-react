...
const WeatherLoaded = "weather/london";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=london&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.londonWeather || !state.londonWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.londonWeather.weather.name,
        temperature: Math.round(state.londonWeather.weather.main.temp - 273),
        description: state.londonWeather.weather.weather[0].description,
    };
};
