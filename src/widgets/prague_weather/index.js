...
const WeatherLoaded = "weather/prague";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=prague&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.pragueWeather || !state.pragueWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.pragueWeather.weather.name,
        temperature: Math.round(state.pragueWeather.weather.main.temp - 273),
        description: state.pragueWeather.weather.weather[0].description,
    };
};
