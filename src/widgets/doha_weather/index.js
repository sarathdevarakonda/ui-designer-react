...
const WeatherLoaded = "weather/doha";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=doha&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.dohaWeather || !state.dohaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.dohaWeather.weather.name,
        temperature: Math.round(state.dohaWeather.weather.main.temp - 273),
        description: state.dohaWeather.weather.weather[0].description,
    };
};
