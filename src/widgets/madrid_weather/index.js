...
const WeatherLoaded = "weather/madrid";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=madrid&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.madridWeather || !state.madridWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.madridWeather.weather.name,
        temperature: Math.round(state.madridWeather.weather.main.temp - 273),
        description: state.madridWeather.weather.weather[0].description,
    };
};
