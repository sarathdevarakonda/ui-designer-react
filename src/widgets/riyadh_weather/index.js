...
const WeatherLoaded = "weather/riyadh";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=riyadh&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.riyadhWeather || !state.riyadhWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.riyadhWeather.weather.name,
        temperature: Math.round(state.riyadhWeather.weather.main.temp - 273),
        description: state.riyadhWeather.weather.weather[0].description,
    };
};
