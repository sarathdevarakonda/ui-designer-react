...
const WeatherLoaded = "weather/moscow";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=moscow&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.moscowWeather || !state.moscowWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.moscowWeather.weather.name,
        temperature: Math.round(state.moscowWeather.weather.main.temp - 273),
        description: state.moscowWeather.weather.weather[0].description,
    };
};
