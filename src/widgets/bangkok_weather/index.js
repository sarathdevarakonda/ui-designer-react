...
const WeatherLoaded = "weather/bangkok";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=bangkok&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.bangkokWeather || !state.bangkokWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.bangkokWeather.weather.name,
        temperature: Math.round(state.bangkokWeather.weather.main.temp - 273),
        description: state.bangkokWeather.weather.weather[0].description,
    };
};
