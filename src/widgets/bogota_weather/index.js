...
const WeatherLoaded = "weather/bogota";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=bogota&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.bogotaWeather || !state.bogotaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.bogotaWeather.weather.name,
        temperature: Math.round(state.bogotaWeather.weather.main.temp - 273),
        description: state.bogotaWeather.weather.weather[0].description,
    };
};
