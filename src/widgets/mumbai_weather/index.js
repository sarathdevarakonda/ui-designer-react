...
const WeatherLoaded = "weather/mumbai";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=mumbai&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.mumbaiWeather || !state.mumbaiWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.mumbaiWeather.weather.name,
        temperature: Math.round(state.mumbaiWeather.weather.main.temp - 273),
        description: state.mumbaiWeather.weather.weather[0].description,
    };
};
