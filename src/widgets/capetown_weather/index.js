...
const WeatherLoaded = "weather/capetown";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            capetownWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=capetown&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.capetownWeather || !state.capetownWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.capetownWeather.weather.name,
        temperature: Math.round(state.capetownWeather.weather.main.temp - 273),
        description: state.capetownWeather.weather.weather[0].description,
    };
};
