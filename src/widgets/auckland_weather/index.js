...
const WeatherLoaded = "weather/auckland";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            aucklandWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=auckland&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.aucklandWeather || !state.aucklandWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.aucklandWeather.weather.name,
        temperature: Math.round(state.aucklandWeather.weather.main.temp - 273),
        description: state.aucklandWeather.weather.weather[0].description,
    };
};
