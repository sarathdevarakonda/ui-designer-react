...
const WeatherLoaded = "weather/caracas";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=caracas&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.caracasWeather || !state.caracasWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.caracasWeather.weather.name,
        temperature: Math.round(state.caracasWeather.weather.main.temp - 273),
        description: state.caracasWeather.weather.weather[0].description,
    };
};
