...
const WeatherLoaded = "weather/dubai";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=dubai&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.dubaiWeather || !state.dubaiWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.dubaiWeather.weather.name,
        temperature: Math.round(state.dubaiWeather.weather.main.temp - 273),
        description: state.dubaiWeather.weather.weather[0].description,
    };
};
