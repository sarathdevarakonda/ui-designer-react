...
const WeatherLoaded = "weather/berlin";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=berlin&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.berlinWeather || !state.berlinWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.berlinWeather.weather.name,
        temperature: Math.round(state.berlinWeather.weather.main.temp - 273),
        description: state.berlinWeather.weather.weather[0].description,
    };
};
