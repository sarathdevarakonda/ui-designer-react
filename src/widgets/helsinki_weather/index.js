...
const WeatherLoaded = "weather/helsinki";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            helsinkiWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=helsinki&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.helsinkiWeather || !state.helsinkiWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.helsinkiWeather.weather.name,
        temperature: Math.round(state.helsinkiWeather.weather.main.temp - 273),
        description: state.helsinkiWeather.weather.weather[0].description,
    };
};
