...
const WeatherLoaded = "weather/oslo";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=oslo&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.osloWeather || !state.osloWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.osloWeather.weather.name,
        temperature: Math.round(state.osloWeather.weather.main.temp - 273),
        description: state.osloWeather.weather.weather[0].description,
    };
};
