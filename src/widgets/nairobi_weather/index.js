...
const WeatherLoaded = "weather/nairobi";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=nairobi&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.nairobiWeather || !state.nairobiWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.nairobiWeather.weather.name,
        temperature: Math.round(state.nairobiWeather.weather.main.temp - 273),
        description: state.nairobiWeather.weather.weather[0].description,
    };
};
