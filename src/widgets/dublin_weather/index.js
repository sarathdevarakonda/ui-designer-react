...
const WeatherLoaded = "weather/dublin";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=dublin&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.dublinWeather || !state.dublinWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.dublinWeather.weather.name,
        temperature: Math.round(state.dublinWeather.weather.main.temp - 273),
        description: state.dublinWeather.weather.weather[0].description,
    };
};
