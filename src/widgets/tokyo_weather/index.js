...
const WeatherLoaded = "weather/tokyo";
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=tokyo&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.tokyoWeather || !state.tokyoWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.tokyoWeather.weather.name,
        temperature: Math.round(state.tokyoWeather.weather.main.temp - 273),
        description: state.tokyoWeather.weather.weather[0].description,
    };
};
