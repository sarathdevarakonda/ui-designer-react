...
const WeatherLoaded = "weather/brasilia";
...
function getWeatherModule() {
    return {
        ...
        reducerMap: {
            brasiliaWeather: weatherReducer,
        },
...
function* loadWeatherData() {
    const url =
        "https://api.openweathermap.org/data/2.5/weather?q=brasilia&APPID=1362c34423375d167d694489b1c74080";
...
const mapStateToProps = state => {
    if (!state.brasiliaWeather || !state.brasiliaWeather.weather) {
        return {
            loading: true,
        };
    }
    ...
    return {
        name: state.brasiliaWeather.weather.name,
        temperature: Math.round(state.brasiliaWeather.weather.main.temp - 273),
        description: state.brasiliaWeather.weather.weather[0].description,
    };
};
